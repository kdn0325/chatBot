...
const ChatBotHome = ({ navigation }: NavigationProp) => {
  ...
  const handleSend = async () => {
    ...
    setData([
      ...data,
      { type: "user", text: textInput, time: Unix_timestamp(time) },
      { type: "bot", text: text, time: Unix_timestamp(time) },
    ]);
    setLoading(false);
    setTextInput("");
  };
    ...
    <Container>
      ...
      <ContentArea>
        <FlatList
          style={{
            borderWidth: 1,
            borderColor: theme.COLORS.BG_GRAY3,
            borderStyle: "solid",
          }}
          ref={flatListRef}
          onContentSizeChange={() => {
          ...
          renderItem={({ item }) => (
            <View style={{ padding: 10 }}>
              {item.type === "user" ? (
                ...
                >
                  <Text
                    style={{ fontWeight: "bold", color: theme.COLORS.BLUE1 }}
                  >
                    나:
                  </Text>
                  <Text
                ...
                >
                  <Text style={{ fontWeight: "bold", color: theme.COLORS.RED }}>
                    ChatGPT:
                  </Text>
                  <Text
                    style={{
