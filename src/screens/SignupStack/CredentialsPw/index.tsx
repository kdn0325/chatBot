...
import {ClearButton, ContentArea, FooterArea, HeadArea, HeaderTxt, InputContainer,TouchButton } from './styles'
...
const CredentialsPw = ({ route, navigation }:NavigationProp) => {
  ...
  return (
        <Container>
            ...
            <ContentArea>
                <InputContainer>
                    <Input title='비밀번호 입력' value={enterPw} onChange={setEnterPw} secureTextEntry={showPwd ? true : false}/>
                ...
                <InputContainer>
                    <Input title='비밀번호 재입력' value={reenterpw} onChange={setReenterPw} secureTextEntry={showPwd ? true : false}/>
                    <TouchButton onPress={() => toggleClick()}>
                        <GlobalImage source={Eyes}/>
                    </TouchButton>
            ...
            <FooterArea>
                <ButtonBox title='계속' disabled={enterPw.length === 0 || reenterpw.length === 0 ? true : false} color={enterPw.length === 0 || reenterpw.length === 0 ? theme.COLORS.BG_GRAY2 : theme.COLORS.BLUE1} fontColor={enterPw.length === 0 || reenterpw.length === 0 ? theme.COLORS.GRAY1 : theme.COLORS.WHITE} pressHandler={pressHandler}/>
            </FooterArea>
        </Container>
  )
}
